...
class ChannelAttention(nn.Module):
    def __init__(self, in_channels, reduction_ratio=16):
        ...
        # else:
        #    raise ValueError("reduction_ratio should be a factor of in_channels to avoid division by zero")
...
class CTNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.rednet(x)
        x = self.cbam(x)
        x = self.out_layer(x).squeeze(-3)
        return x


class REDNet(nn.Module):
    def __init__(self, num_layers=20, num_features=8):
...
if __name__ == '__main__':
    model = CTNet()
    info = Summary.summary(model,(1,200,400),batch_size=-1,device="cpu")
    print(info)