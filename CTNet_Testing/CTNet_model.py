...
class CTNet(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(0,1,1):
            plt.figure()
            plt.ion()
            plt.imshow(x[0,i,:,:].abs()/torch.max(x[0,i,:,:].abs()),extent=[0, 4, 0, 160 // 2],  aspect='auto')
            plt.xticks([])
            plt.yticks([])
       ...
       # print('x.cuda',x.shape)
        ...
        plt.imshow(x[0, :, :].abs() / torch.max(x[0, :, :].abs()),extent=[0, 4, 0, 160 // 2],  aspect='auto')
        plt.xticks([])
        plt.yticks([])
        plt.gca().invert_yaxis()
        plt.xlabel('Time')
        ...
        # data3 = mat['TFR']
        # print('data', data3.shape)
        # ideal2 = torch.from_numpy(data3)
        # loss_l2 = torch.pow((x - ideal2), 2)
